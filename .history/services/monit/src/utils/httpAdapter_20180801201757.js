...
function settle(resolve, reject, response) {
  if (typeof response.status !== 'undefined' && response.status <= 400) {
    ...
    reject(normalizeError(`Request failed with status code ${response.status}`));
  }
}
...
module.exports = (config) => {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', (err) => {
      if (req.aborted) return;
      reject(normalizeError(err, config, null, req));
    });
    ...
    if (config.timeout && !timer) {
      timer = setTimeout(() => {
        req.abort();
        reject(normalizeError(`ECONNABORTED:: timeout of ${config.timeout}ms exceeded`, { timeout: config.timeout }, TIME_OUT_STATUS));
      }, config.timeout);
    }
